...
- C++ structs and classes mapped from Slice structs and classes can now be printed using
`operator<<(ostream&, const T&)`. This operator prints the type name and all the field names and values; see the
[Slice/print] test for examples.
  - You can also implement your own custom printing by applying metadata `["cpp:custom-print"]` to your Slice struct or
    class.
