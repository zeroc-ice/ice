...
# Regenerate the common Ice test certificates (the certs used to run the test suite with the
# --protocol ssl option). Run via makecerts-docker.sh (recommended) or directly on a host that has
# openssl, keytool, and the Bouncy Castle provider installed.
#
# Usage: ./makecerts.sh [common]
#
# The certs/configuration certificates (used by the IceSSL/configuration test) are consolidated in
# this tree but are not yet regenerated by this script; see README.md.
