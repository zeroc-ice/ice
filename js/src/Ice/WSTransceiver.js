    ...
    WSTransceiver = class
    {
        ...
        initialize(readBuffer, writeBuffer)
        {
            ...
            if(this._state === StateNeedConnect)
            {
                ...
                {
                    this._exception = new Ice.ConnectFailedException("connect failed", { cause: e });
                    throw this._exception;
                }
        ...
        socketClosed(event)
        {
            ...
            if (this._state < StateConnected)
            {
                if(event.type === "close")
                {
                    this._exception = new Ice.ConnectFailedException(event.code, event.reason);
                }
                else
                {
                    this._exception = new Ice.ConnectFailedException(0, event);
                }
                this._connectedCallback();
            }
            else if(event.type === "close")
            {
                ...
                {
                    this._exception = new Ice.ConnectionLostException();
                }
                else
                {
                    this._exception = new Ice.SocketException(event.code, event.reason);
                }

            }
            else
            {
                // A generic Event generated by onerror (which doesn't have a code).
                this._exception = new Ice.ConnectionLostException();
            }
