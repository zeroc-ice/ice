...
export class Client extends TestHelper {
    ...
    async allTestsWithController(controller: Test.ControllerPrx) {
        async function connect(prx: Ice.ObjectPrx) {
            let nRetry = 10;
            while (--nRetry > 0) {
                try {
        ...
        await Ice.Promise.delay(500);
        {
            ...
            try {
                //
                // setTimeout can be unpredictable slow in Safari when the process
                // enter background, we increase the expected time that destroy can takes
                // for these browsers.
                //
                const t: number = TestHelper.isSafari() ? 30000 : mult * 2000;
