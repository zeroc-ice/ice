...
public class ControllerApp extends Application
{
    ...
    {
        @SuppressWarnings("unchecked") TestSuiteBundle(String name, ClassLoader loader) throws ClassNotFoundException
        {
            _loader = loader;
        ...
        ClassLoader getClassLoader() { return _loader; }
    ...
    class AndroidLogger implements Logger
    {
        ...
        AndroidLogger(String prefix) { _prefix = prefix; }

        @Override public void print(String message) { Log.d(TAG, message); }

        @Override public void trace(String category, String message) { Log.v(category, message); }
        ...
        @Override public Logger cloneWithPrefix(String s) { return new AndroidLogger(s); }
    }
    ...
    synchronized public void setIpv4Address(String address) { _ipv4Address = address; }
...
isEmulator()
{
    return Build.FINGERPRINT.startsWith("google/sdk_gphone") || Build.FINGERPRINT.startsWith("generic") ||
        ...
        (Build.BRAND.startsWith("generic") && Build.DEVICE.startsWith("generic")) || Build.PRODUCT.equals("google_sdk");
}
...
class ControllerI
{
    ...
    {
        registry.ice_pingAsync().whenCompleteAsync((r1, e1) -> {
            if (e1 != null)
            {
            ...
            else
            {
                ...
                registry.setProcessControllerAsync(processController).whenCompleteAsync((r2, e2) -> {
                    if (e2 != null)
                    {
...
class ControllerHelperI extends Thread implements test.TestHelper.ControllerHelper
{
    ...
    public void run()
    {
        try
        {
            ...
            _helper.setWriter(new Writer() {
                ...
                @Override public void write(char[] buf, int offset, int count) { _out.append(buf, offset, count); }
            });
    ...
    public String getOutput() { return _out.toString(); }
    ...
    synchronized private void waitReady(int timeout) throws Test.Common.ProcessFailedException
    {
    ...
    synchronized private int waitSuccess(int timeout) throws Test.Common.ProcessFailedException
    {
        long now = Time.currentMonotonicTimeMillis();
        while (!_completed)
        {
...
{
    public Test.Common.ProcessPrx
    start(final String testsuite, final String exe, String[] args, com.zeroc.Ice.Current current)
        throws Test.Common.ProcessFailedException
    {
        println("starting " + testsuite + " " + exe + "... ");
        String className = "test." + testsuite.replace("/", ".") + "." + exe.substring(0, 1).toUpperCase(Locale.ROOT) +
                           exe.substring(1);
        try
        {
    ...
    public String getHost(String protocol, boolean ipv6, com.zeroc.Ice.Current current)
    {
        ...
        {
            synchronized (ControllerApp.this) { return ipv6 ? _ipv6Address : _ipv4Address; }
        }
    }
}
...
{
    public ProcessI(ControllerHelperI controllerHelper) { _controllerHelper = controllerHelper; }
