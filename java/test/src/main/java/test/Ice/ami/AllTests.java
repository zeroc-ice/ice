        ...
        if (p.ice_getConnection() != null && p.supportsAMD() && !bluetooth) {
            ...
            }
            {
                ...
                while (!done && maxQueue < 50) {
                    ...
                    if (!Util.getInvocationFuture(p.closeAsync(CloseMode.GracefullyWithWait))
                            .isSent()) {
                        for (int i = 0; i < maxQueue; i++) {
                            InvocationFuture<Void> r =
            ...
            {
                //
                // Local case: start an operation and then close the connection gracefully on the
                // client
                // side
                // without waiting for the pending invocation to complete. There will be no retry
                // and we
                // expect the
                ...
                try {
                    ...
                    test(
                            ((com.zeroc.Ice.ConnectionManuallyClosedException) ex.getCause())
                                    .graceful);
                } catch (Throwable ex) {
                    test(false);
                }
                ...
                //
                // Remote case: the server closes the connection gracefully, which means the
                // connection
