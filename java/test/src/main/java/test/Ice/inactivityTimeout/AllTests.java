...
public class AllTests {
  static void allTests(test.TestHelper helper) {
    ...
    testServerInactivityTimeout(proxyString3s, communicator.getProperties(), helper.getWriter());
  ...
  private static void testServerInactivityTimeout(
    ...
    try (var communicator = Util.initialize(initData)) {
      ...
      // The inactivity timeout is 5s on the client side and 3s on the server side. 4 seconds tests
      // the server side.
      try {
        Thread.sleep(4000);
      } catch (InterruptedException ex) {
  ...
  private static void testWithOutstandingRequest(
    ...
    output.write(
        "testing the inactivity timeout with an outstanding " + onewayString + " request... ");
    output.flush();
    ...
    if (oneway) {
      // With a oneway invocation, the inactivity timeout on the client side shut down the first
      // connection.
      test(connection2 != connection);
    } else {
