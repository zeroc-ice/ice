  ...
  public void run(String[] args) {
    ...
    try (com.zeroc.Ice.Communicator communicator = initialize(properties)) {
      ...
      com.zeroc.Ice.ObjectAdapter oa =
          communicator.createObjectAdapterWithEndpoints("MyOA2", "tcp -h localhost");
  ...
  private void testInterceptorExceptions(MyObjectPrx prx) {
    ...
    for (ExceptionPoint e : exceptions) {
      ...
      }
      {
        ...
        // Force the last batch request to be dispatched by the server thread using invocation
        // timeouts
