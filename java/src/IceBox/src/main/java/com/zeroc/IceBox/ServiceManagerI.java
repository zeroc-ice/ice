...
public class ServiceManagerI implements ServiceManager {
  ...
  public void startService(String name, Current current)
    ...
    try {
      info.service.start(
          name, info.communicator == null ? _sharedCommunicator : info.communicator, info.args);
      started = true;
  ...
  private synchronized void start(
    ...
    if (classDir != null) {
      try {
        ...
        throw new FailureException(
            "ServiceManager: invalid path in plug-in entry point `" + classDir + "'", ex);
    ...
    try {
      ...
      try {
        //
        // If the service class provides a constructor that accepts an com.zeroc.Ice.Communicator
        // argument,
  ...
  private void servicesStarted(
    ...
    if (!services.isEmpty()) {
      ...
      for (final ServiceObserverPrx observer : observers) {
        observer
            .servicesStartedAsync(servicesArray)
            .exceptionally(
                ex -> {
                  observerFailed(observer, ex);
                  return null;
  ...
  private void servicesStopped(
    ...
    if (!services.isEmpty()) {
      ...
      for (final ServiceObserverPrx observer : observers) {
        observer
            .servicesStoppedAsync(servicesArray)
            .exceptionally(
                ex -> {
                  observerFailed(observer, ex);
                  return null;
