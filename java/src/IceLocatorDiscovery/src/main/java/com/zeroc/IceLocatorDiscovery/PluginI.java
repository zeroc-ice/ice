  ...
  private static class LocatorI implements com.zeroc.Ice.BlobjectAsync {
    ...
    public synchronized void foundLocator(com.zeroc.Ice.LocatorPrx locator) {
      ...
      if (!_instanceName.isEmpty() && !locator.ice_getIdentity().category.equals(_instanceName)) {
        if (_traceLevel > 2) {
          StringBuffer s =
              new StringBuffer("ignoring locator reply: instance name doesn't match\n");
          ...
          locator
              ...
              .warning(
                  "received Ice locator with different instance name:\n"
                      ...
                      + " instance names are deployed and the property"
                      + " `IceLocatorDiscovery.InstanceName'is not set.");
        }
        return;
      }
    ...
    public synchronized void invoke(com.zeroc.Ice.LocatorPrx locator, Request request) {
      if (request != null && _locator != null && _locator != locator) {
        ...
        request.invoke(
            _voidLocator); // Don't retry to find a locator before the retry delay expires
      } else {
        _locator = null;
        ...
        if (!_pending) // No request in progress
        {
          ...
          try {
            ...
            _future =
                _timer.schedule(_retryTask, _timeout, java.util.concurrent.TimeUnit.MILLISECONDS);
          } catch (com.zeroc.Ice.LocalException ex) {
            if (_traceLevel > 0) {
            ...
            synchronized (LocatorI.this) {
              ...
              if (_pendingRetryCount > 0) {
                ...
                try {
                  ...
                  _future =
                      _timer.schedule(
                          _retryTask, _timeout, java.util.concurrent.TimeUnit.MILLISECONDS);
                  return;
                } catch (com.zeroc.Ice.LocalException ex) {
                }
                _pendingRetryCount = 0;
  ...
  public void initialize() {
    ...
    if (properties.getProperty(_name + ".Reply.Endpoints").isEmpty()) {
      properties.setProperty(
          _name + ".Reply.Endpoints", "udp -h " + (intf.isEmpty() ? "*" : "\"" + intf + "\""));
    }
    ...
    _locator =
        new LocatorI(_name, LookupPrx.uncheckedCast(lookupPrx), properties, instanceName, voidLoc);
