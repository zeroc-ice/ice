...
class SSLEngine
{
    ...
    void initialize()
    {
        ...
        if (protocols.length != 0)
        {
            ...
            for (String prot : protocols)
            {
                ...
                if (s.equals("SSL3") || s.equals("SSLV3"))
                {
                    ...
                    s.equals("TLS") || s.equals("TLS1") || s.equals("TLSV1") || s.equals("TLS1_0") ||
                    s.equals("TLSV1_0"))
                {
                    l.add("TLSv1");
                }
        ...
        if (passwordCallbackClass.length() > 0)
        {
            ...
            {
                throw new PluginInitializationException(
                    "IceSSL: unable to instantiate password callback class " + passwordCallbackClass,
                    ex);
            }
        }
        ...
        if (_context == null)
        {
            try
            {
                ...
                // by the JVM implementation. Other possibilities include "PKCS12" and "BKS".
                //
                final String truststoreType = properties.getPropertyWithDefault(
                    prefix + "TruststoreType",
                ...
                if (_keystoreStream != null || keystorePath.length() > 0)
                {
                    ...
                    if (!alias.isEmpty())
                    {
                        ...
                        {
                            keyManagers[i] = new X509KeyManagerI(
                        ...
                        else
                        {
                            trustManagers = new javax.net.ssl.TrustManager[] {new javax.net.ssl.X509TrustManager(){
                                @Override public void checkClientTrusted(X509Certificate[] chain, String authType)
                                    throws CertificateException{throw new CertificateException("no trust anchors");
                        }
...
Certificate[]
getVerifiedCertificateChain(Certificate[] chain)
{
    if (_validator == null)
    {
...
createSSLEngine(boolean incoming, String host, int port)
{
...
void
traceConnection(String desc, javax.net.ssl.SSLEngine engine, boolean incoming)
{
...
void
verifyPeer(String address, ConnectionInfo info, String desc)
{
    //
    ...
    {
        String msg =
            (info.incoming ? "incoming" : "outgoing") + " connection rejected by certificate verifier\n" + desc;
        if (_securityTraceLevel >= 1)
        {
            _logger.trace(_securityTraceCategory, msg);
        }
...
void
trustManagerFailure(boolean incoming, CertificateException ex) throws CertificateException
{
    if (_verifyPeer == 0)
    {
        if (_securityTraceLevel >= 1)
...
openResource(String path) throws java.io.IOException
{
    ...
    {
        stream = com.zeroc.IceInternal.Util.openResource(
