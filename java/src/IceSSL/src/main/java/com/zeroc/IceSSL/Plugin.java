...
public interface Plugin extends com.zeroc.Ice.Plugin {
  /**
   * Establishes the SSL context. The context must be established before plug-in is initialized.
   * Therefore, the application must set the property <code>Ice.InitPlugins</code> to zero, call
   * <code>setContext</code> to set the context, and finally invoke
   * PluginManager.initializePlugins().
   *
   * <p>If an application supplies its own SSL context, the plug-in skips its normal property-based
   * configuration.
   *
  ...
  /**
   * Returns the SSL context. Use caution when modifying the returned value: changes made to this
   * value do not affect existing connections.
   *
  ...
  /**
   * Supplies an input stream for the keystore. Calling this method causes IceSSL to ignore the
   * <code>IceSSL.Keystore</code> property.
   *
  ...
  /**
   * Supplies an input stream for the truststore. Calling this method causes IceSSL to ignore the
   * <code>IceSSL.Truststore</code> property. It is legal to supply the same input stream as the one
   * for {@link #setKeystoreStream}, in which case IceSSL uses the certificates contained in the
   * keystore.
   *
  ...
  /**
   * Adds an input stream for the random number seed. You may call this method multiple times if
   * necessary.
   *
