  ...
  private void runReadThread() {
    ...
    try {
      ...
      while (true) {
        synchronized (this) {
          //
          // If we've read too much data, wait until the application consumes some before we read
          // again.
          //
      ...
      //
      // Mark as ready for reading so that the Ice run time will invoke read() and we can report the
      // exception.
      //
      _readyCallback.ready(SocketOperation.Read, true);
