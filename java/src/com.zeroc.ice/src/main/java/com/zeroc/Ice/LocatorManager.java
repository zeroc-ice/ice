...
final class LocatorManager {
    ...
    public LocatorInfo get(LocatorPrx loc) {
        ...
        synchronized (this) {
            ...
            if (info == null) {
                // Rely on locator identity for the adapter table.
                // We want to have only one table per locator (not one per locator proxy).
    ...
    private final HashMap<LocatorKey, LocatorTable> _locatorTables = new HashMap<>();
