...
final class WSTransceiver implements Transceiver {
        ...
        try {
            if (_state == StateConnected) {
                ...
                if (!_incoming) {
                    ...
                    _rand.nextBytes(key);
                    _key = StringUtil.encodeBase64(key);
                    out.append(_key + "\r\n\r\n"); // EOM
    ...
    public int closing(boolean initiator, LocalException reason) {
        ...
        if (s == StateClosingRequestPending && _closingInitiator) {
            ...
            // we assume we didn't initiate the close and we send the close frame now.
            // This is to ensure that if both peers close the connection at the same time we
            // don't hang having both peers waiting for the close frame of the other.
            assert (!initiator);
    ...
    private void handleRequest(Buffer responseBuffer) {
        ...
        try {
            byte[] decodedKey = StringUtil.decodeBase64(key);
        ...
        try {
            ...
            out.append(StringUtil.encodeBase64(hash) + "\r\n\r\n"); // EOM
    ...
    private void handleResponse() {
        ...
        try {
            ...
            final MessageDigest sha1 = MessageDigest.getInstance("SHA1");
            sha1.update(input.getBytes(_ascii));
            if (!val.equals(StringUtil.encodeBase64(sha1.digest()))) {
