...
class LocatorRegistryI implements com.zeroc.Ice.LocatorRegistry {
  public LocatorRegistryI(com.zeroc.Ice.Communicator com) {
    _wellKnownProxy =
        com.stringToProxy("p").ice_locator(null).ice_router(null).ice_collocationOptimized(true);
  }
  ...
  synchronized FindAdapterResult findAdapter(String adapterId) {
    ...
    if (s != null) {
      ...
      if (prx != null) {
        ...
        r.returnValue =
            prx.ice_endpoints(endpoints.toArray(new com.zeroc.Ice.Endpoint[endpoints.size()]));
        return r;
      }
    }
    r.isReplicaGroup = false;
