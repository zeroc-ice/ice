...
class LookupI implements Lookup
{
    abstract private class Request<T, Ret> implements Runnable
    {
        ...
        boolean retry() { return --_retryCount >= 0; }
    ...
    private class AdapterRequest extends Request<String, com.zeroc.Ice.ObjectPrx>
    {
        ...
        @Override boolean retry() { return _proxies.size() == 0 && --_retryCount >= 0; }
        ...
        @Override void finished(com.zeroc.Ice.ObjectPrx proxy)
        {
        ...
        @Override public void run() { adapterRequestTimedOut(this); }

        @Override protected void invokeWithLookup(String domainId, LookupPrx lookup, LookupReplyPrx lookupReply)
        {
    ...
    {
        ObjectRequest(com.zeroc.Ice.Identity id, int retryCount) { super(id, retryCount); }

        void response(com.zeroc.Ice.ObjectPrx proxy) { finished(proxy); }
        ...
        @Override public void run() { objectRequestTimedOut(this); }
    ...
    public void findObjectById(
        ...
        com.zeroc.IceDiscovery.LookupReplyPrx reply,
    ...
    synchronized void
    foundAdapter(String adapterId, String requestId, com.zeroc.Ice.ObjectPrx proxy, boolean isReplicaGroup)
    {
        AdapterRequest request = _adapterRequests.get(adapterId);
        if (request != null && request.getRequestId().equals(requestId)) // Ignore responses from old requests
        {
