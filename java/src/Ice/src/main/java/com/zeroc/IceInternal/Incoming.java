...
public final class Incoming implements com.zeroc.Ice.Request {
  public Incoming(
      Instance instance,
  ...
  public <T> CompletionStage<OutputStream> setResultFuture(CompletionStage<T> f, Write<T> write) {
    final OutputStream cached =
        getAndClearCachedOutputStream(); // If an output stream is cached, re-use it
  ...
  public <T extends MarshaledResult> CompletionStage<OutputStream> setMarshaledResultFuture(
      CompletionStage<T> f) {
    final CompletableFuture<OutputStream> r = new CompletableFuture<OutputStream>();
    f.whenComplete(
        (result, ex) -> {
  ...
  public OutputStream getAndClearCachedOutputStream() {
    if (_response) {
      ...
      // Don't consume unnecessarily the output stream if we are dispatching a oneway request.
      return null;
    }
  }
  ...
  public static OutputStream createResponseOutputStream(Current current) {
    OutputStream os =
        new OutputStream(current.adapter.getCommunicator(), Protocol.currentProtocolEncoding);
    ...
    out.print(
        "\nfacet: "
            + com.zeroc.IceUtilInternal.StringUtil.escapeString(
                _current.facet, "", _instance.toStringMode()));
    out.print("\noperation: " + _current.operation);
    if (_current.con != null) {
      try {
        for (ConnectionInfo connInfo = _current.con.getInfo();
            connInfo != null;
  ...
  private void handleException(Throwable exc, boolean amd) {
    ...
    try {
      ...
      //
      // Raised by CompletableFuture.get(). The inner exception caused the future to complete
      // exceptionally.
