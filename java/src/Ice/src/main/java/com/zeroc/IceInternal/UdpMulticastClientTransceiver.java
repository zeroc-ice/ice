...
final class UdpMulticastClientTransceiver implements Transceiver {
  ...
  UdpMulticastClientTransceiver(
      ProtocolInstance instance, InetSocketAddress addr, String mcastInterface, int mcastTtl) {
    ...
    try {
      ...
      //
      // NOTE: Setting the multicast interface before performing the connect is important for some
      // systems
      // such as macOS.
      //
      ...
      if (_size < _newSize) {
        ...
        if (!winfo.sndWarn || winfo.sndSize != _newSize) {
          _instance
              .logger()
              .warning(
                  "UDP send buffer size: requested size of " + _newSize + " adjusted to " + _size);
  ...
  //
  // The maximum IP datagram size is 65535. Subtract 20 bytes for the IP header and 8 bytes for the
  // UDP header
