...
public interface BlobjectAsync extends com.zeroc.Ice.Object {
   ...
   * @param current The Current object, which provides important information about the request, such
   *     as the identity of the target object and the name of the operation.
   * @return A completion stage that eventually completes with the result of the invocation, an
   *     instance of <code>Ice_invokeResult</code>. If the operation completed successfully, set the
   *     <code>returnValue</code> member to <code>true</code> and the <code>outParams</code> member
   *     to the encoded results. If the operation raises a user exception, you can throw it directly
   *     from <code>ice_invokeAsync</code>, or complete the future by setting the <code>returnValue
   *     </code> member to <code>false</code> and the <code>outParams</code> member to the encoded
   *     user exception.
   * @throws UserException A user exception raised by this method will be marshaled as the result of
   *     the invocation.
   */
  ...
  default CompletionStage<OutputStream> _iceDispatch(
    ...
    final OutputStream cached =
        in.getAndClearCachedOutputStream(); // If an output stream is cached, re-use it
    s.whenComplete(
        (result, ex) -> {
          if (ex != null) {
