...
final class LoggerAdminLoggerI implements LoggerAdminLogger, Runnable
{
    ...
    @Override public String getPrefix() { return _localLogger.getPrefix(); }
    ...
    @Override public com.zeroc.Ice.Object getFacet() { return _loggerAdmin; }
    ...
    @Override public void destroy()
    {
        ...
        if (thread != null)
        {
            ...
            {
                synchronized (this) { _sendLogThread = thread; }
        ...
        for (;;)
        {
            ...
            for (com.zeroc.Ice.RemoteLoggerPrx p : job.remoteLoggers)
            {
                ...
                try
                {
                    ...
                    //
                    p.logAsync(job.logMessage).whenComplete((Void v, Throwable ex) -> {
                        if (ex != null)
                        {
                            ...
                            {
                                _loggerAdmin.deadRemoteLogger(p, _localLogger, (com.zeroc.Ice.LocalException)ex, "log");
                            }
                            else
                            {
                                _loggerAdmin
                                    .deadRemoteLogger(p, _localLogger, new com.zeroc.Ice.UnknownException(ex), "log");
                            }
                        }
                        else
                        {
                            if (_loggerAdmin.getTraceLevel() > 1)
                            {
                                _localLogger.trace(
                                    _traceCategory,
                                    "log on `" + p.toString() + "' completed successfully");
                            }
                        }
                    });
                }
    ...
    com.zeroc.Ice.Logger getLocalLogger() { return _localLogger; }
    ...
    static private long now() { return java.util.Calendar.getInstance().getTimeInMillis() * 1000; }
