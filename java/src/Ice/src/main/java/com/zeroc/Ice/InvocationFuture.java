...
public abstract class InvocationFuture<T> extends CompletableFuture<T> {
   ...
   * request to the client-side transport. If the transport cannot accept the request, the Ice run
   * time queues the request for later transmission. This method returns true if, at the time it is
   * called, the request has been written to the local transport (whether it was initially queued or
   * not). Otherwise, if the request is still queued, this method returns false.
   *
   ...
   * If the request was initially queued, this method returns false (independent of whether the
   * request is still in the queue or has since been written to the client-side transport).
   *
  ...
  /**
   * Returns a future that completes when the entire request message has been accepted by the
   * transport and executes the given action using the default executor. The boolean value indicates
  ...
  /**
   * Returns a future that completes when the entire request message has been accepted by the
   * transport and executes the given action using the executor. The boolean value indicates whether
   * the message was sent synchronously.
   *
   * @param action Executed when the future is completed successfully or exceptionally.
