...
final class LoggerAdminI implements com.zeroc.Ice.LoggerAdmin {
              ...
              (Void v, Throwable ex) -> {
                if (ex != null) {
                  if (ex instanceof com.zeroc.Ice.LocalException) {
                    deadRemoteLogger(
                        remoteLogger, _logger, (com.zeroc.Ice.LocalException) ex, "init");
                  } else {
                    deadRemoteLogger(
                        remoteLogger, _logger, new com.zeroc.Ice.UnknownException(ex), "init");
                  }
                } else {
                  if (_traceLevel > 1) {
                    _logger.trace(
  ...
  private static RemoteLoggerPrx changeCommunicator(
      RemoteLoggerPrx prx, com.zeroc.Ice.Communicator communicator) {
    if (prx == null) {
      return null;
    }
  ...
  private static void copyProperties(
      String prefix, com.zeroc.Ice.Properties from, com.zeroc.Ice.Properties to) {
    for (java.util.Map.Entry<String, String> p : from.getPropertiesForPrefix(prefix).entrySet()) {
      to.setProperty(p.getKey(), p.getValue());
    }
  }

  private static com.zeroc.Ice.Communicator createSendLogCommunicator(
  ...
  private final java.util.Map<com.zeroc.Ice.Identity, RemoteLoggerData> _remoteLoggerMap =
