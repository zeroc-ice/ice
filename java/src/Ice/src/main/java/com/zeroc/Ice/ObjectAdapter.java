...
public interface ObjectAdapter {
  ...
  /**
   * Activate all endpoints that belong to this object adapter. After activation, the object adapter
   * can dispatch requests received through its endpoints.
   *
   * @see #hold
   * @see #deactivate
   */
  ...
  /**
   * Temporarily hold receiving and dispatching requests. The object adapter can be reactivated with
   * the {@link #activate} operation.
   *
   * <p class="Note">Holding is not immediate, i.e., after {@link #hold} returns, the object adapter
   * might still be active for some time. You can use {@link #waitForHold} to wait until holding is
   * complete.
   *
   * @see #activate
   * @see #deactivate
   ...
   * deactivated object adapter raise {@link ObjectAdapterDeactivatedException} however, attempts to
   * {@link #deactivate} an already deactivated object adapter are ignored and do nothing. Once
   ...
   *
   * <p class="Note">After {@link #deactivate} returns, no new requests are processed by the object
   * adapter. However, requests that have been started before {@link #deactivate} was called might
   * still be active. You can use {@link #waitForDeactivate} to wait for the completion of all
   * requests for this object adapter.
   *
   * @see #activate
   * @see #hold
   * @see #waitForDeactivate
   ...
   * object adapter has not yet been deactivated, destroy implicitly initiates the deactivation and
   * waits for it to finish. Subsequent calls to destroy are ignored. Once destroy has returned, it
   * is possible to create another object adapter with the same name.
   *
   * @see #deactivate
   * @see #waitForDeactivate
   ...
   * several Ice objects by registering the servant with multiple identities. Adding a servant with
   * an identity that is in the map already throws {@link AlreadyRegisteredException}.
   *
  ...
  /**
   * Add a default servant to handle requests for a specific category. Adding a default servant for
   * a category for which a default servant is already registered throws {@link
   * AlreadyRegisteredException}. To dispatch operation calls on servants, the object adapter tries
   * to find a servant for a given Ice object identity and facet in the following order:
   *
   * <ol>
   *   <li>The object adapter tries to find a servant for the identity and facet in the Active
   *       Servant Map.
   *   <li>If no servant has been found in the Active Servant Map, the object adapter tries to find
   *       a default servant for the category component of the identity.
   *   <li>If no servant has been found by any of the preceding steps, the object adapter tries to
   *       find a default servant for an empty category, regardless of the category contained in the
   *       identity.
   ...
   *
   * @param id The identity of the Ice object that is implemented by the servant. If the servant
   *     implements multiple Ice objects, {@link #remove} has to be called for all those Ice
   *     objects. Removing an identity that is not in the map throws {@link NotRegisteredException}.
  ...
  /**
   * Remove all facets with the given identity from the Active Servant Map. The operation completely
   * removes the Ice object, including its default facet. Removing an identity that is not in the
   * map throws {@link NotRegisteredException}.
   *
  ...
  /**
   * Remove the default servant for a specific category. Attempting to remove a default servant for
   * a category that is not registered throws {@link NotRegisteredException}.
   *
  ...
  /**
   * Look up a servant in this object adapter's Active Servant Map by the identity of the Ice object
   * it implements.
   *
   ...
   * @return The servant that implements the Ice object with the given identity, or null if no such
  ...
  /**
   * Add a Servant Locator to this object adapter. Adding a servant locator for a category for which
   * a servant locator is already registered throws {@link AlreadyRegisteredException}. To dispatch
   * operation calls on servants, the object adapter tries to find a servant for a given Ice object
   * identity and facet in the following order:
   *
   * <ol>
   *   <li>The object adapter tries to find a servant for the identity and facet in the Active
   *       Servant Map.
   *   <li>If no servant has been found in the Active Servant Map, the object adapter tries to find
   *       a servant locator for the category component of the identity. If a locator is found, the
   ...
   * @param category The category for which the Servant Locator can locate servants, or an empty
   *     string if the Servant Locator does not belong to any specific category.
   * @return The Servant Locator, or throws {@link NotRegisteredException} if no Servant Locator was
  ...
  /**
   * Create a direct proxy for the object with the given identity. The returned proxy contains this
   ...
   * configured with an adapter id, the return value refers to the adapter id. Otherwise, the return
