...
public class ConnectRequestHandler
    ...
    public synchronized RequestHandler connect(com.zeroc.Ice._ObjectPrxI proxy) {
        if (!initialized()) {
            _proxies.add(proxy);
        }
    ...
    @Override
    public synchronized com.zeroc.Ice.ConnectionI getConnection() {
        //
        // First check for the connection, it's important otherwise the user could first get a
        // connection
        // and then the exception if he tries to obtain the proxy cached connection mutiple times
        // (the
    ...
    private void flushRequestsImpl() {
        ...
        for (ProxyOutgoingAsyncBase outAsync : _requests) {
            try {
                ...
                _reference
                        ...
                        .removeRequestHandler(_reference, this);
        ...
        if (_reference.getCacheConnection() && exception == null) {
            ...
            if (_reference.getInstance().queueRequests()) {
                _requestHandler =
                        new QueueRequestHandler(_reference.getInstance(), _requestHandler);
            }
