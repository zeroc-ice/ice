...
public final class LocatorInfo {
    ...
    private abstract class Request {
        ...
        protected void exception(Exception ex) {
            synchronized (this) {
                _locatorInfo.finishRequest(
                        _ref, _wellKnownRefs, null, ex instanceof com.zeroc.Ice.UserException);
                _exception = ex;
                notifyAll();
            }
            for (RequestCallback callback : _callbacks) {
    ...
    public com.zeroc.Ice.LocatorRegistryPrx getLocatorRegistry() {
        ...
        synchronized (this) {
            ...
            _locatorRegistry =
                    locatorRegistry
                            .ice_locator(null)
                            .ice_endpointSelection(com.zeroc.Ice.EndpointSelectionType.Ordered);
            return _locatorRegistry;
        }
    }
            ...
            if (instance.traceLevels().location >= 1) {
                ...
                s.append(
                        com.zeroc.Ice.Util.identityToString(
                                ref.getIdentity(), instance.toStringMode()));
                instance.initializationData()
                        .logger
                        .trace(instance.traceLevels().locationCat, s.toString());
            }
    ...
    private void finishRequest(
        ...
        if (!ref.isWellKnown()) {
            if (proxy != null
                    && !((com.zeroc.Ice._ObjectPrxI) proxy)._getReference().isIndirect()) {
            ...
            if (proxy != null
                    && !((com.zeroc.Ice._ObjectPrxI) proxy)._getReference().isWellKnown()) {
                // Cache the well-known object reference.
                _table.addObjectReference(
                        ref.getIdentity(), ((com.zeroc.Ice._ObjectPrxI) proxy)._getReference());
            } else if (notRegistered) {
