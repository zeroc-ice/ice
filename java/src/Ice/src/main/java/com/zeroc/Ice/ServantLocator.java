...
public interface ServantLocator {
   ...
   * operation includes that user exception, the client receives that user exception; otherwise, the
   * client receives {@link UnknownUserException}. If <code>locate</code> throws any exception, the
   * Ice run time does <em>not</em>call <code>finished</code>.
   *
   ...
   * operation can be used for cleanup purposes after a request. <code>finished</code> can throw any
   * user exception. If it does, that exception is marshaled back to the client. If the Slice
   ...
   * operation and <code>finished</code> throw an exception, the exception thrown by <code>finished
