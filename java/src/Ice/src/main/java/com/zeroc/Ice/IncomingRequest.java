...
public final class IncomingRequest {
  ...
  /**
   * Gets the number of bytes in the request. These are all the bytes starting with the identity of
   * the target.
   */
   ...
   * @param inputStream The input stream buffer over the incoming Ice protocol request message. The
   *     stream is positioned at the beginning of the request header - the next data to read is the
  ...
  public IncomingRequest(
    ...
    current =
        new Current(
            adapter, connection, identity, facet, operation, mode, ctx, requestId, encoding);
