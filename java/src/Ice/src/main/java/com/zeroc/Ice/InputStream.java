...
public class InputStream {
   ...
   * exception. If the stream was initialized with a communicator, the communicator's resolver will
   * be used by default.
   *
  ...
  /**
   * Returns a blob of bytes representing an encapsulation. The encapsulation's encoding version is
   ...
   * @return The optional value (if any).
   */
  public <T extends java.io.Serializable> java.util.Optional<T> readSerializable(
      int tag, Class<T> cl) {
    if (readOptional(tag, OptionalFormat.VSize)) {
   ...
   * @return The optional value (if any).
   */
  public <T extends ObjectPrx> java.util.Optional<T> readProxy(
      int tag, java.util.function.Function<ObjectPrx, T> cast) {
    if (readOptional(tag, OptionalFormat.FSize)) {
      skip(4);
  ...
  private static final class EncapsDecoder10 extends EncapsDecoder {
    ...
    String startSlice() {
      ...
      //
      if (_sliceType
          == SliceType.ValueSlice) // For exceptions, the type ID is always encoded as a string
      {
      ...
      if (_current.sliceType == SliceType.ValueSlice) {
            ...
            == Protocol.FLAG_HAS_TYPE_ID_COMPACT) // Must be checked 1st!
        {
          ...
          _current.compactId = _stream.readSize();
                ...
                & (Protocol.FLAG_HAS_TYPE_ID_INDEX | Protocol.FLAG_HAS_TYPE_ID_STRING))
            != 0) {
          _current.typeId =
          ...
          if (v == null) {
            ...
            if (_compactIdResolver != null) {
              try {
                ...
                throw new MarshalException(
                    "exception in compact ID resolver for ID " + _current.compactId, ex);
              }
            }
  ...
  private static final END_OF_BUFFER_MESSAGE = "Attempting to unmarshal past the end of the buffer.";
}
