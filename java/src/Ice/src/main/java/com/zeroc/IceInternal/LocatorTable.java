...
final class LocatorTable {
    ...
    synchronized Reference getObjectReference(
            com.zeroc.Ice.Identity id, int ttl, Holder<Boolean> cached) {
        if (ttl == 0) // Locator cache disabled.
        {
            cached.value = false;
            return null;
    ...
    synchronized void addObjectReference(com.zeroc.Ice.Identity id, Reference ref) {
        _objectTable.put(id, new ReferenceTableEntry(Time.currentMonotonicTimeMillis(), ref));
    }

    synchronized Reference removeObjectReference(com.zeroc.Ice.Identity id) {
    ...
    private java.util.Map<com.zeroc.Ice.Identity, ReferenceTableEntry> _objectTable =
            new java.util.HashMap<>();
}
