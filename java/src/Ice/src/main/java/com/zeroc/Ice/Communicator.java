...
public final class Communicator implements AutoCloseable {
  ...
  /**
   * Destroy the communicator. This Java-only method overrides close in java.lang.AutoCloseable and
   ...
   * shutdown was called might still be active. You can use {@link #waitForShutdown} to wait for the
   ...
   * main thread, which then waits until some other thread calls {@link #shutdown}. After shut-down
   * is complete, the main thread returns and can do some cleanup work before it finally calls
   * {@link #destroy} to shut down the client functionality, and then exits the application.
   *
   * @see #shutdown
   * @see #destroy
   ...
   * operation throws ParseException. Refer to the Ice manual for a detailed description of the
   * syntax supported by stringified proxies.
   *
  ...
  /**
   * Convert a set of proxy properties into a proxy. The "base" name supplied in the <code>property
   * </code> argument refers to a property containing a stringified proxy, such as <code>
   * MyProxy=id:tcp -h localhost -p 10000</code>. Additional properties configure local settings for
   * the proxy, such as <code>MyProxy.PreferSecure=1</code>. The "Properties" appendix in the Ice
   ...
   *     object adapter. When set to a non-null value all Ice.SSL configuration properties are
   ...
   *     through Ice.SSL configuration properties. Passing null is equivalent to calling {@link
   ...
   * router. To disable the default router, null can be used. Note that this operation has no effect
   * on existing proxies. You can also set a router for an individual proxy by calling the operation
   * <code>ice_router</code> on the proxy.
   *
   ...
   * will use this default locator. To disable the default locator, null can be used. Note that this
   * operation has no effect on existing proxies or object adapters. You can also set a locator for
   * an individual proxy by calling the operation <code>ice_locator</code> on the proxy, or for an
   * object adapter by calling {@link ObjectAdapter#setLocator} on the object adapter.
   *
   ...
   * Ice.Admin.InstanceName}/admin, or {UUID}/admin when <code>Ice.Admin.InstanceName</code> is not
   * set. If Ice.Admin.DelayCreation is 0 or not set, getAdmin is called by the communicator
   * initialization, after initialization of all plugins.
   *
   * @return A proxy to the main ("") facet of the Admin object, or a null proxy if no Admin object
   *     is configured.
   * @see #createAdmin
   */
