 ...
 * of this interface is provided by the Ice communicator on initialization to the {@link
 * CommunicatorObserver} object set with the communicator initialization data. The Ice communicator
 * calls {@link CommunicatorObserver#setObserverUpdater} to provide the observer updater. This
 * interface can be used by add-ins implementing the {@link CommunicatorObserver} interface to
...
public interface ObserverUpdater {
   ...
   * and its object adapters. When called, this method goes through all the connections and for each
   * connection {@link CommunicatorObserver#getConnectionObserver} is called. The implementation of
   * getConnectionObserver has the possibility to return an updated observer if necessary.
   */
