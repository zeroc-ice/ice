...
public interface UserExceptionFactory {
  /**
   * Instantiate a user exception with the given Slice type ID (such as <code>::Module::MyException
   * </code>) and throw it. If the implementation does not throw an exception, the Ice run time will
   * fall back to using its default behavior for instantiating the user exception.
   *
