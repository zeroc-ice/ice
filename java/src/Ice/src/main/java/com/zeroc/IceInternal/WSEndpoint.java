...
final class WSEndpoint extends EndpointI {
  ...
  public void connectors_async(
      com.zeroc.Ice.EndpointSelectionType selType, final EndpointI_connectors callback) {
  ...
  public EndpointI.ExpandHostResult expandHost() {
    ...
    if (result.publish != null) {
      result.publish =
          result.publish == _delegate ? this : new WSEndpoint(_instance, result.publish, _resource);
    }
    return result;
  }
  ...
  protected boolean checkOption(String option, String argument, String endpoint) {
    switch (option.charAt(1)) {
      case 'r':
        {
          if (argument == null) {
            throw new com.zeroc.Ice.ParseException("no argument provided for -r option in endpoint '" + endpoint + _delegate.options() + "'");
          }
          _resource = argument;
          return true;
        }
