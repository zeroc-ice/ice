...
final class UdpTransceiver implements Transceiver
{
    ...
    @Override public synchronized void checkSendSize(Buffer buf)
    {
        //
    ...
    @Override public synchronized void setBufferSize(int rcvSize, int sndSize) { setBufSize(rcvSize, sndSize); }
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            if (sizeRequested < (_udpOverhead + Protocol.headerSize))
            {
                _instance.logger().warning(
                    "Invalid " + prop + " value of " + sizeRequested + " adjusted to " + dfltSize);
            ...
            if (sizeRequested != dfltSize)
            {
                ...
                if (sizeSet < sizeRequested)
                {
                    ...
                    {
                        _instance.logger().warning(
                            "UDP " + direction + " buffer size: requested size of " + sizeRequested + " adjusted to " +
                            sizeSet);
