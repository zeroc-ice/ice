...
final class UdpTransceiver implements Transceiver {
    ...
    public EndpointI bind() {
        if (_addr.getAddress().isMulticastAddress()) {
            ...
            if (_port == 0) {
                _mcastAddr =
                        new java.net.InetSocketAddress(_mcastAddr.getAddress(), _addr.getPort());
            }
        ...
        while (true) {
            try {
                if (_state == StateConnected) {
                    ...
                    if (_peerAddr == null) {
                        throw new com.zeroc.Ice
                                .SocketException(); // No peer has sent a datagram yet.
                    }
            ...
            if (sizeRequested != dfltSize) {
                ...
                if (sizeSet < sizeRequested) {
                    BufSizeWarnInfo winfo =
                            _instance.getBufSizeWarn(com.zeroc.Ice.UDPEndpointType.value);
                    if ((isSnd && (!winfo.sndWarn || winfo.sndSize != sizeRequested))
                        ...
                        if (isSnd) {
                            _instance.setSndBufSizeWarn(
                                    com.zeroc.Ice.UDPEndpointType.value, sizeRequested);
                        } else {
                            _instance.setRcvBufSizeWarn(
                                    com.zeroc.Ice.UDPEndpointType.value, sizeRequested);
                        }
                    }
                }
