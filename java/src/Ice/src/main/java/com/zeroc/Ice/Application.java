...
/**
 * Utility base class that makes it easy to correctly initialize and finalize the Ice run time, as
 * well as handle signals. Unless the application specifies a logger, <code>Application</code>
 * installs a per-process logger that logs to the standard error output.
 *
 * <p>Applications must create a derived class that implements the {@link #run} method.
 *
 * <p>A program can contain only one instance of this class.
 *
 * @see #run
 * @see Communicator
 * @see Logger
...
public abstract class Application {
    /** Initializes an instance that calls {@link Communicator#destroy} if a signal is received. */
    public Application() {}
    ...
    /**
     * The application must call <code>main</code> after it has instantiated the derived class.
     * <code>
     * main</code> creates a communicator, establishes the specified signal policy, and, once <code>
     * run</code> returns, destroys the communicator.
     *
     * <p>The method prints an error message for any exception that propagates out of <code>run
     * </code> and ensures that the communicator is destroyed correctly even if <code>run</code>
     * completes abnormally.
     *
     * @param appName The name of the application. This parameter is used to initialize the value of
     *     the <code>Ice.ProgramName</code> property.
     * @param args The arguments for the application (as passed to <code>Main(String[])</code> by
     *     the operating system.
     * @return The value returned by <code>run</code>. If <code>run</code> terminates with an
     *     exception, the return value is non-zero.
     */
        ...
        if (configFile != null) {
            try {
                ...
                Util.getProcessLogger()
                        .error("unknown exception: " + com.zeroc.IceInternal.Ex.toString(ex));
                return 1;
            }
        }
    ...
    public final int main(String appName, String[] args, InitializationData initializationData) {
        ...
        if (!initData.properties.getProperty("Ice.ProgramName").isEmpty()
            ...
            Util.setProcessLogger(
                    new LoggerI(initData.properties.getProperty("Ice.ProgramName"), ""));
        }
    ...
    /**
     * Called once the communicator has been initialized. The derived class must implement <code>run
     * </code>, which is the application's starting method.
     *
     * @param args The argument vector for the application. <code>Application</code> scans the
     *     argument vector passed to <code>main</code> for options that are specific to the Ice run
     *     time and removes them; therefore, the vector passed to <code>run</code> is free from
     *     Ice-related options and contains only options and arguments that are
     *     application-specific.
     * @return The <code>run</code> method should return zero for successful termination, and
     *     non-zero otherwise. <code>Application.main</code> returns the value returned by <code>run
     *     </code>.
     */
    public abstract int run(String[] args);

    /**
     * Returns the value of <code>appName</code> that is passed to <code>main</code> (which is also
     * the the value of <code>Ice.ProgramName</code>). This method is useful mainly for error
     * messages that include the application name. Because <code>appName</code> is a static method,
     * it is available from anywhere in the program.
     *
    ...
    /**
     * Returns the communicator for the application. Because <code>communicator</code> is a static
     * method, it permits access to the communicator from anywhere in the program. Note that, as a
     * consequence, you cannot have more than one instance of <code>Application</code> in a program.
     *
    ...
    /**
     * Instructs <code>Application</code> to call {@link Communicator#destroy} on receipt of a
     * signal. This is default signal handling policy established by the default constructor.
     *
    ...
    /**
     * Instructs <code>Application</code> to call {@link Communicator#shutdown} on receipt of a
     * signal.
     *
    ...
    /**
     * Installs a custom shutdown hook. The implementation of the shutdown hook can do whatever
     * cleanup is necessary to shut down the application. The hook is unregistered once <code>run
     * </code> returns. Note that the hook must obey the rules for shutdown hooks; specifically, it
     * must not call <code>exit</code>.
     *
    ...
    /**
     * Determines whether the application shut down intentionally or was forced to shut down by the
     * JVM. This is useful for logging purposes.
     *
     * @return <code>true</code> if a shutdown hook caused the communicator to shut down; false
     *     otherwise.
     */
     ...
     * @hidden
     */
    protected static final java.lang.Object _mutex = new java.lang.Object();
