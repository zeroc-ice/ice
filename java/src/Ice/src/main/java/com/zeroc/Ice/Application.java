...
public abstract class Application {
  ...
  /**
   * The application must call <code>main</code> after it has instantiated the derived class. <code>
   ...
   *
   * <p>The method prints an error message for any exception that propagates out of <code>run</code>
   * and ensures that the communicator is destroyed correctly even if <code>run</code> completes
   * abnormally.
   *
   ...
   * @param args The arguments for the application (as passed to <code>Main(String[])</code> by the
   *     operating system.
   ...
   *     Ice-related options and contains only options and arguments that are application-specific.
   * @return The <code>run</code> method should return zero for successful termination, and non-zero
   *     otherwise. <code>Application.main</code> returns the value returned by <code>run</code>.
   */
  public abstract int run(String[] args);

  /**
   * Returns the value of <code>appName</code> that is passed to <code>main</code> (which is also
   * the the value of <code>Ice.ProgramName</code>). This method is useful mainly for error messages
   * that include the application name. Because <code>appName</code> is a static method, it is
   * available from anywhere in the program.
   *
