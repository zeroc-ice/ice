...
class ServerSubEditor extends CommunicatorSubEditor {
    ServerSubEditor(Editor mainEditor) {
        ...
        _options.setToolTipText(
                "<html>Command-line arguments for this server.<br>Use whitespace as separator; use"
                        + " double-quotes around arguments containing whitespaces</html>");

        _user.getDocument().addDocumentListener(_mainEditor.getUpdateListener());
        _user.setToolTipText(
                "<html>Run the server using this user account.<br>This feature is only available on"
                        + " Unix/Linux, when the IceGrid node runs as root.</html>");
