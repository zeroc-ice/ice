...
{
    public Action get(int index) { return _array[index]; }
    ...
    ApplicationActions(boolean popup)
    {
        ...
        _array[TreeNode.NEW_TEMPLATE_SERVER_ICEBOX] =
            new AbstractAction(popup ? "New IceBox Server Template" : "IceBox Server Template") {
                @Override public void actionPerformed(ActionEvent e)
                {
                    if (apply(_target))
                    {
        ...
        _array[TreeNode.COPY] = new AbstractAction("Copy") {
            @Override public void actionPerformed(ActionEvent e) { _target.copy(); }
        };
        ...
        _array[TreeNode.DELETE] = new AbstractAction("Delete") {
            @Override public void actionPerformed(ActionEvent e) { _target.delete(); }
        };

        _array[TreeNode.SHOW_VARS] = new AbstractAction("Show Variables") {
            @Override public void actionPerformed(ActionEvent e) { _target.getCoordinator().showVars(); }
        };
        ...
        _array[TreeNode.SUBSTITUTE_VARS] = new AbstractAction("Substitute Variables") {
            @Override public void actionPerformed(ActionEvent e) { _target.getCoordinator().substituteVars(); }
        };
    ...
    static private boolean apply(TreeNode target) { return target.getRoot().getPane().applyUpdates(false); }
