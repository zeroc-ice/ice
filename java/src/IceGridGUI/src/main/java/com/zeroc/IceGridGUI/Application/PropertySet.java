...
class PropertySet extends TreeNode {
  ...
  public Editor getEditor() {
    if (_editor == null) {
      if (_inServerInstance) {
        _editor =
            (PropertySetEditor) getRoot().getEditor(ServerInstancePropertySetEditor.class, this);
      } else {
  ...
  void write(XMLWriter writer) throws java.io.IOException {
    if (!_ephemeral) {
      writePropertySet(
          writer, _unsubstitutedId, _inServerInstance ? "service" : "id", _descriptor, null, null);
    }
  }
