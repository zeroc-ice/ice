...
public abstract class Communicator extends TreeNode {
                          ...
                          () -> {
                            ...
                            ((CommunicatorEditor) getEditor())
                                .setRuntimeProperties(
                                    (java.util.SortedMap<String, String>) result,
                                    Communicator.this);
                          });
                    } else if (ex instanceof com.zeroc.Ice.ObjectNotExistException
                        || ex instanceof com.zeroc.Ice.FacetNotExistException) {
                      SwingUtilities.invokeLater(
                          () ->
                              getRoot()
                                  .getCoordinator()
                                  .getStatusBar()
                                  .setText(prefix + " Admin not available."));
                    } else {
                      amiFailure(
                          prefix, "Failed to retrieve the properties for " + getDisplayName(), ex);
                    }
                  });
        });
  }
              ...
              .whenComplete(
                  (result, ex) -> {
                    if (ex == null) {
                      ...
                      amiFailure(
                          prefix, "Failed to retrieve the metrics for " + getDisplayName(), ex);
                    }
                  });
        })) {
  ...
  public java.util.List<MetricsView> getMetrics() {
    return new java.util.ArrayList<>(_metrics);
  }

  protected abstract java.util.concurrent.CompletableFuture<com.zeroc.Ice.ObjectPrx>
      getAdminAsync();
  ...
  private boolean provideAdmin(
      final String prefix, final java.util.function.Consumer<com.zeroc.Ice.ObjectPrx> consumer) {
              ...
              (admin, adminEx) -> {
                if (adminEx == null && admin != null) {
                  try {
                    ...
                    || adminEx instanceof com.zeroc.Ice.ObjectNotExistException) {
                  SwingUtilities.invokeLater(
                      () ->
                          getRoot()
                              .getCoordinator()
                  ...
                  amiFailure(
                      prefix,
                      "Failed to retrieve the Admin proxy for " + getServerDisplayName(),
                      adminEx);
                }
              });
    } catch (com.zeroc.Ice.LocalException e) {
