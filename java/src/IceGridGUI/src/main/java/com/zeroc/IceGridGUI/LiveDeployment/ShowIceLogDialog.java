...
class ShowIceLogDialog extends JDialog
{
    private class MenuBar extends JMenuBar
    {
        private MenuBar()
        {
            ...
            fileMenu.add(new AbstractAction("Close") {
                @Override public void actionPerformed(ActionEvent e) { close(true); }
            });
            ...
            editMenu.add(new AbstractAction("Preferences...") {
                @Override public void actionPerformed(ActionEvent e) { new LogPrefsDialog(ShowIceLogDialog.this); }
            });
            editMenu.addSeparator();
            editMenu.add(new AbstractAction("Filter...") {
                @Override public void actionPerformed(ActionEvent e) { new LogFilterDialog(ShowIceLogDialog.this); }
            });
        }
    }
    ...
    {
        @Override public synchronized void init(String prefix, LogMessage[] logMessages, Current current)
        {
            // Ignore prefix
            ...
            {
                _rowCount =
                    logMessages.length + _queue.size() < _maxRows ? logMessages.length + _queue.size() : _maxRows;
        ...
        @Override public synchronized void log(LogMessage message, Current current)
        {
            if (_destroyed)
            {
                // Nothing to do
        ...
        private synchronized void stop() { _destroyed = true; }
        ...
        int initialMessages)
    {
        ...
        addWindowListener(new java.awt.event.WindowAdapter() {
            @Override public void windowClosing(java.awt.event.WindowEvent e) { close(true); }
        });
        ...
        _play = new AbstractAction("Play") {
            @Override public void actionPerformed(ActionEvent e) { play(); }
        };
        ...
        pack();
        Utils
            .restoreWindowBounds(this, _preferences, "IceLogDialog", _parent.getRoot().getCoordinator().getMainFrame());
    ...
    private void play()
    {
        if (_remoteLogger == null)
        {
            ...
            _remoteLoggerPrx =
                RemoteLoggerPrx.uncheckedCast(_parent.getRoot().getCoordinator().addCallback(_remoteLogger, id, ""));
    ...
    int getMaxMessages() { return _maxMessages; }

    int getInitialMessages() { return _initialMessages; }
    ...
    LogMessageType[] getMessageTypeFilter() { return _messageTypeFilter; }

    String[] getTraceCategoryFilter() { return _traceCategoryFilter; }
    ...
    private static String renderMessage(String msg) { return msg.replace("\n", " "); }
