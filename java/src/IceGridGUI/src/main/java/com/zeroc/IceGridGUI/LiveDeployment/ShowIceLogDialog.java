...
class ShowIceLogDialog extends JDialog {
                  ...
                  if (file != null) {
                    ...
                    try {
                      ...
                      for (Object p : _tableModel.getDataVector()) {
                        ...
                        String txt =
                            "\""
                                + renderDate((java.util.Date) row.elementAt(0))
                                + "\","
  ...
  private static class LogMessageTypeRenderer extends DefaultTableCellRenderer {
    @Override
    public void setValue(Object value) {
      if (value == null) {
        setText("");
  ...
  ShowIceLogDialog(
    ...
    Utils.restoreWindowBounds(
        this, _preferences, "IceLogDialog", _parent.getRoot().getCoordinator().getMainFrame());
  ...
  private void play() {
    if (_remoteLogger == null) {
      ...
      String id =
          _loggerAdmin.ice_getIdentity().name + "-" + java.util.UUID.randomUUID().toString();
      ...
      try {
        _loggerAdmin
            .attachRemoteLoggerAsync(
                _remoteLoggerPrx, _messageTypeFilter, _traceCategoryFilter, _initialMessages)
            .whenComplete(
                (result, ex) -> {
                  if (ex == null) {
  ...
  private void stop(boolean detach) {
    if (_remoteLogger != null) {
      if (detach) {
        final String prefix =
            "Detaching remote logger from " + _loggerAdmin.ice_getIdentity().name + "...";
        _parent.getRoot().getCoordinator().getStatusBar().setText(prefix);
