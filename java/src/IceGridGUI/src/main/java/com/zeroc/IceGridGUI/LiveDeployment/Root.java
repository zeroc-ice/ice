...
public class Root extends Communicator {
  ...
  public Root(Coordinator coordinator) {
    ...
    _messageSizeMax =
        computeMessageSizeMax(_coordinator.getProperties().getPropertyAsInt("Ice.MessageSizeMax"));
  ...
  public void shutdownRegistry() {
    ...
    try {
      ...
      admin
          .shutdownRegistryAsync(_replicaName)
          .whenComplete(
              (result, ex) -> {
  ...
  public void objectInit(ObjectInfo[] objects) {
    for (ObjectInfo info : objects) {
      _objects.put(
          info.proxy.ice_getCommunicator().identityToString(info.proxy.ice_getIdentity()), info);
    }
  }
