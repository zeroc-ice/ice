...
public class GraphView extends JFrame implements MetricsFieldContext, Coordinator.IGraphView
{
    class TimeFormatter extends StringConverter<java.lang.Number>
    {
        ...
        @Override public synchronized String toString(Number timestamp)
        {
        ...
        @Override public synchronized Number fromString(String time)
        {
            try
            {
        ...
        Action preferences = new AbstractAction("Preferences") {
            @Override public void actionPerformed(ActionEvent event)
            {
                ...
                SpinnerNumberModel refreshPeriod =
                    new SpinnerNumberModel(getRefreshPeriod(), _minRefreshPeriod, _maxRefreshPeriod, 1);
    ...
    private boolean loadPreferences()
    {
        Preferences preferences =
            Utils.restoreWindowBounds(this, _preferences, "GraphView", _coordinator.getMainFrame());
        if (preferences == null)
        {
            return false;
        }
    ...
    @Override public void addSeries(final MetricsViewTransferableData data)
    {
        //
                ...
                for (MetricsCell j : i.getValue())
                {
                    if (columns.get(j.getField().getFieldName()) == null)
                    {
                        ...
                        final MetricsRow row =
                            new MetricsRow(data.info, j, color, new XYChart.Series<Number, Number>());
    ...
    private void
    addData(final MetricsViewInfo info, final Map<String, com.zeroc.Ice.IceMX.Metrics[]> data, final long timestamp)
    {
        //
        ...
        enqueueJFX(() -> {
                    ...
                    for (Map.Entry<String, MetricsRow> k : j.getValue().entrySet())
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                row.series.peek().getData().add(new XYChart.Data<Number, Number>(timestamp, value));
                            }
            ...
            //
            enqueueSwing(() -> {
                _legendModel.fireTableChanged(new TableModelEvent(
                    _legendModel,
                    0,
    ...
    synchronized private void startRefresh()
    {
        if (_refreshFuture == null)
        {
            _refreshFuture = _coordinator.getScheduledExecutor().scheduleAtFixedRate(() -> {
                ...
                synchronized (GraphView.this) { metrics = new java.util.HashSet<>(_series.keySet()); }
    ...
    @Override public synchronized int getRefreshPeriod() { return _refreshPeriod; }
    ...
    synchronized String getDateFormat() { return _dateFormat; }
    ...
    synchronized private int getMaximumSamples() { return _samples; }
    ...
    {
        @Override public String getColumnName(int col) { return _columnNames[col]; }

        @Override public int getRowCount() { return _rows.size(); }

        @Override public int getColumnCount() { return _columnNames.length; }
        ...
        {
            if (col < _columnNames.length && (_columnNames[col].equals("Show") || _columnNames[col].equals("Scale") ||
        ...
        @Override public void setValueAt(final Object value, int rowIndex, int columnIndex)
        {
            if (isCellEditable(rowIndex, columnIndex))
            {
                ...
                else if (_columnNames[columnIndex].equals("Color"))
                {
                    ...
                    // example: #ff00aa
                    //
                    row.color = "#" + String.format("%02X", color.getRed()) + String.format("%02X", color.getGreen()) +
        ...
        public MetricsRow[] getRows() { return _rows.values().toArray(new MetricsRow[_rows.size()]); }
    ...
    void updateScaleFactor(final XYChart.Series<Number, Number> series, final double s1, final double s2)
    {
        //
        // Must run in JavaFX thread.
        //
    ...
    @SuppressWarnings("rawtypes") static class DecimalRenderer extends DefaultListCellRenderer
    {
        public DecimalRenderer(ListCellRenderer renderer) { this._renderer = renderer; }
    ...
    public class ColorEditor extends AbstractCellEditor implements TableCellEditor, ActionListener
    {
        ...
        @Override public Object getCellEditorValue() { return _currentColor; }
    ...
    private final static String[] _columnNames = new String[] {
        "Show",
        "Component",
        "Metrics View Name",
        "Metrics Name",
    ...
    // Field name
    //
    private final Map<MetricsViewInfo, Map<String, Map<String, Map<String, MetricsRow>>>> _series = new HashMap<>();
    ...
    private final Double[] _scales = new Double[] {
        0.000000001d,
        0.00000001d,
        0.0000001d,
        0.000001d,
