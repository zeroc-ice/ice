...
def filter(f)
    ...
    # SChannel is Windows-only (case-insensitive check)
    if f.downcase.include?("schannel")
        return false
    end
    if RUBY_PLATFORM =~ /darwin/ and f.downcase.include?("openssl")
        return false
    end
    if !(RUBY_PLATFORM =~ /darwin/) and f.downcase.include?("securetransport")
         return false
    end
    return true
end
