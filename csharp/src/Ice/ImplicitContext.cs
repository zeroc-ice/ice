...
namespace Ice
{
    ...
    internal class SharedImplicitContext : ImplicitContext
    {
        ...
        public override void Write(Dictionary<string, string> prxContext, OutputStream os)
        {
            if (prxContext.Count == 0)
            {
                lock (this)
                {
                    ContextHelper.OutputStreamWriter(os, _context);
                }
            }
            else
            {
                ...
                ContextHelper.OutputStreamWriter(os, ctx);
            }
        }
    ...
    internal class PerThreadImplicitContext : ImplicitContext
    {
        ...
        public override void Write(Dictionary<string, string> prxContext, OutputStream os)
        {
            ...
            {
                ContextHelper.OutputStreamWriter(os, prxContext);
            }
            else if (prxContext.Count == 0)
            {
                ContextHelper.OutputStreamWriter(os, threadContext);
            }
            else
            {
                var combined = new Dictionary<string, string>(prxContext);
                ...
                ContextHelper.OutputStreamWriter(os, combined);
            }
        }
