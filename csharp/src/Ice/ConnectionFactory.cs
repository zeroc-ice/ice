...
namespace ZeroC.Ice
{
    ...
    internal sealed class OutgoingConnectionFactory : IConnectionManager, IAsyncDisposable
    {
        ...
        // We keep a map of the connectors that recently resulted in a transport failure while establishing a
        // connection. This is used to influence the selection of connectors when creating new connections. Connectors
        // with recent failures are tried last.
            ...
            CancellationToken cancel)
        {
            ...
            foreach (Endpoint endpoint in endpoints)
            {
                ...
                catch when (!ReferenceEquals(endpoint, last) || connectors.Count > 0)
                {
                    ...
                    // and keep going, otherwise let the exception go throw the caller might want to retry
                    // with fresh endpoints.
                }
            }
