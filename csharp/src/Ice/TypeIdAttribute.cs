...
namespace Ice
{
    ...
    public sealed class TypeIdAttribute : Attribute
    {
        ...
        /// <summary>GetTypeId retrieves the type ID from a type with the attribute TypeId</summary>
        /// <param name="type">The class or interface generated by the Slice compiler.</param>
        /// <returns>The type ID, or null if type does not carry the TypeId attribute.</returns>
        ...
        /// <summary>GetAllTypeIds retrieves the type ID from a type and from all its base types.
        ...
        /// <param name="type">The class or interface generated by the Slice compiler.</param>
        /// <returns>An array of type IDs.</returns>
        public static string[] GetAllTypeIds(Type type)
        {
            if (type.IsInterface)
            {
                var result = new List<string>();
                if (GetTypeId(type) is string firstTypeId)
                {
                    result.Add(firstTypeId);
                    var interfaces = type.GetInterfaces();
                    foreach (var p in interfaces)
                    {
                        if (GetTypeId(p) is string typeId)
                        {
                            result.Add(typeId);
                        }
                    }
