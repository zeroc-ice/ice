...
public static class BZip2
{
    ...
    public static Buffer compress(Buffer buf, int headerSize, int compressionLevel)
    {
        ...
        {
            throw new CompressionException($"BZ2_bzBuffToBuffCompress failed: {getBZ2Error(rc)}");
        }
    ...
    public static Buffer uncompress(Buffer buf, int headerSize, int messageSizeMax)
    {
        ...
        if (rc < 0)
        {
            throw new CompressionException($"BZ2_bzBuffToBuffDecompress failed: {getBZ2Error(rc)}");
        }
