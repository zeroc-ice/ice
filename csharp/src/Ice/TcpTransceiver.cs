    ...
    internal sealed class TcpTransceiver : ITransceiver
    {
        ...
        public async ValueTask InitializeAsync(CancellationToken cancel)
        {
            if (_addr != null)
            {
                ...
                {
                    throw new ConnectionRefusedException(ex, RetryPolicy.AfterDelay(TimeSpan.Zero), _endpoint);
                }
            ...
            IPAddress? sourceAddr)
        {
            ...
            Socket = Network.CreateSocket(false, (_proxy != null ? _proxy.Address : _addr).AddressFamily, _endpoint);
            try
            {
