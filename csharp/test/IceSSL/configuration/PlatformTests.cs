...
public class PlatformTests
{
    ...
    private static void clientValidatesServerUsingValidationCallback(TestHelper helper, string certificatesPath)
    {
        ...
        {
            RemoteCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => certificate.Subject == serverCertificate.Subject
        };
    ...
    private static void serverValidatesClientUsingValidationCallback(TestHelper helper, string certificatesPath)
    {
        ...
        var serverOptions = new SslServerAuthenticationOptions
        {
            ...
            RemoteCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => certificate.Subject == clientCertificate.Subject,
        };
        using var serverCommunicator = createServer(serverOptions, helper);
    ...
    private static void serverHotCertificateReload(TestHelper helper, string certificatesPath)
    {
        ...
        var trustedRootCertificatesCA1 = new X509Certificate2(Path.Combine(certificatesPath, "cacert1.pem"));
        var trustedRootCertificatesCA2 = new X509Certificate2(Path.Combine(certificatesPath, "cacert2.pem"));

        var serverState = new ServerState(Path.Combine(certificatesPath, "s_rsa_ca1.p12"));
...

        {
            var clientOptions = new SslClientAuthenticationOptions
            {
                RemoteCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => certificate.Issuer == trustedRootCertificatesCA1.Subject
            };
            using var clientCommunicator = createClient(clientOptions);
...

        {
            ...
            var clientOptions = new SslClientAuthenticationOptions
            {
                RemoteCertificateValidationCallback = (sender, certificate, chain, sslPolicyErrors) => certificate.Issuer == trustedRootCertificatesCA2.Subject
            };
            using var clientCommunicator = createClient(clientOptions);
        ...
        serverState.reloadCertificate(Path.Combine(certificatesPath, "s_rsa_ca2.p12"));
    ...
    {
        string certificatesPath = Path.Combine(testDir, "..", "certs");
