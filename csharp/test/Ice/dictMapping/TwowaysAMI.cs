...
public static class TwowaysAMI
{
    ...
    internal static async Task twowaysAMI(Test.MyInterfacePrx p)
    {
        {
            ...
            Test.MyInterface_OpNVResult result = await p.opNVAsync(i);
...

        {
            ...
            Test.MyInterface_OpNRResult result = await p.opNRAsync(i);
            test(Internal.DictionaryExtensions.DictionaryEqual(i, result.o));
            test(Internal.DictionaryExtensions.DictionaryEqual(i, result.returnValue));
        }

        {
            ...
            Test.MyInterface_OpNDVResult result = await p.opNDVAsync(i);
...

        {
            ...
            Test.MyInterface_OpNDRResult result = await p.opNDRAsync(i);
            foreach (string key in i.Keys)
            {
                test(Internal.DictionaryExtensions.DictionaryEqual(i[key], result.o[key]));
                test(Internal.DictionaryExtensions.DictionaryEqual(i[key], result.returnValue[key]));
...

        {
            ...
            Test.MyInterface_OpNDAISResult result = await p.opNDAISAsync(i);
            foreach (string key in i.Keys)
            {
...

        {
            ...
            Test.MyInterface_OpNDGISResult result = await p.opNDGISAsync(i);
            foreach (string key in i.Keys)
            {
                test(Enumerable.SequenceEqual(i[key], result.o[key]));
                test(Enumerable.SequenceEqual(i[key], result.returnValue[key]));
...

        {
            ...
            Test.MyInterface_OpNDASSResult result = await p.opNDASSAsync(i);
            foreach (string key in i.Keys)
            {
                test(Enumerable.SequenceEqual(i[key], result.o[key]));
                test(Enumerable.SequenceEqual(i[key], result.returnValue[key]));
...

        {
            ...
            Test.MyInterface_OpNDGSSResult result = await p.opNDGSSAsync(i);
            foreach (string key in i.Keys)
            {
                test(Enumerable.SequenceEqual(i[key], result.o[key]));
                test(Enumerable.SequenceEqual(i[key], result.returnValue[key]));
