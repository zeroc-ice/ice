classdef (Sealed, Hidden) NotFoundSliceLoaderDecorator < Ice.SliceLoader
    ...
    methods
        function obj = NotFoundSliceLoaderDecorator(decoratee, cacheSize, logger)
            % Constructor for NotFoundSliceLoaderDecorator
            %
            % Parameters:
            %   decoratee - The SliceLoader to decorate.
            %   cacheSize - The maximum number of type IDs that can be cached.
            %   logger - The logger used to warn when the cache is full. It's empty when Ice.Warn.SliceLoader is set to
            %            0.
