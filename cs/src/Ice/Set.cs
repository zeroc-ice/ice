...
public class Set : ICollection, ICloneable
{
	...
	if(loadFactor > 1)
	{
	    throw new ArgumentOutOfRangeException("loadfactor", loadFactor, "load factor must not be greater than 1.0");
	}
    ...
    private Entry
    FindEntry(int hash, object value)
    {
    ...
    private void
    AddEntry(int hash, object value)
    {
    ...
    private bool
    RemoveEntry(int hash, object value)
    {
	Entry prev = null;
	Entry cursor = _table[hash];
	while(cursor != null)
