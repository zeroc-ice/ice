...
public sealed class Connection : EventHandler
{
    ...
    public void
    sendRequest(Outgoing og, bool oneway)
    {
	lock(this)
	{
	    if(_exception != null)
    ...
    public void
    sendAsyncRequest(OutgoingAsync og)
    {
	lock(this)
	{
	    if(_exception != null)
    ...
    private void
    warning(string msg, System.Exception ex)
    {
	_logger.warning(msg + ":\n" + ex.StackTrace.ToString() + _transceiver.ToString());
    }
    
    private Incoming
    getIncoming(bool response, byte compress)
    {
	Incoming inc = null;
