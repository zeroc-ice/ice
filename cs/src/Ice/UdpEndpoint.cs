...
sealed class UdpEndpoint : Endpoint
{
    ...
    public
    UdpEndpoint(Instance instance, string ho, int po, bool co)
    {
	_instance = instance;
	_host = ho;
	_port = po;
    ...
    {
	string s = "udp";
	s += " -v " + (_protocolMajor < 0?(int) _protocolMajor + 255:_protocolMajor) + "." + (_protocolMinor < 0?(int) _protocolMinor + 255:_protocolMinor);
	s += " -e " + (_encodingMajor < 0?(int) _encodingMajor + 255:_encodingMajor) + "." + (_encodingMinor < 0?(int) _encodingMinor + 255:_encodingMinor);
	s += " -h " + _host + " -p " + _port;
	if(_connect)
	{
	    s += " -c";
