...
IceUtil::stringToWstring(const string& v, const StringConverterPtr& converter, const WstringConverterPtr& wConverter)
{
    ...
    if (!v.empty())
    {
        ...
        wConverterWithDefault->fromUTF8(
            reinterpret_cast<const Byte*>(tmp.data()), reinterpret_cast<const Byte*>(tmp.data() + tmp.size()), target);
    }
    return target;
}
...
IceUtil::UTF8ToNative(const string& str, const IceUtil::StringConverterPtr& converter)
{
    ...
    string tmp;
    converter->fromUTF8(
        reinterpret_cast<const Byte*>(str.data()), reinterpret_cast<const Byte*>(str.data() + str.size()), tmp);
    return tmp;
}
...
IceUtilInternal::fromUTF32(const vector<unsigned int>& source)
{
    ...
    if (!source.empty())
    {
        ...
        try
        {
            ...
            result = vector<Byte>(
                reinterpret_cast<const Byte*>(bs.data()), reinterpret_cast<const Byte*>(bs.data()) + bs.length());
        }
        catch (const std::range_error& ex)
        {
            throw IllegalConversionException(__FILE__, __LINE__, ex.what());
