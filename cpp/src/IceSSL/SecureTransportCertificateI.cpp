...
namespace
{
    ...
    chrono::system_clock::time_point getX509Date(SecCertificateRef cert, CFTypeRef key)
    {
        ...
        return chrono::system_clock::time_point(
            chrono::seconds(static_cast<long long>(kCFAbsoluteTimeIntervalSince1970 + seconds)));
    }
...
SecureTransportCertificateI::encode() const
{
    ...
    auto start = CFDataGetBytePtr(cert.get());
    auto end = start + CFDataGetLength(cert.get());
    vector<byte> data(reinterpret_cast<const byte*>(start), reinterpret_cast<const byte*>(end));
    ostringstream os;
...
IceSSL::SecureTransport::Certificate::decode(const std::string& encoding)
{
    ...
    UniqueRef<CFDataRef> certdata(
        CFDataCreate(kCFAllocatorDefault, reinterpret_cast<uint8_t*>(&data[0]), static_cast<CFIndex>(data.size())));
