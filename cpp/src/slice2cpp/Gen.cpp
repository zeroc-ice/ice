...
namespace
{
    ...
    string createResultType(const OperationPtr& p, const string& scope, int useWstring)
    {
        assert(useWstring == 0 || useWstring == TypeContextUseWstring);

        TypePtr ret = p->returnType();
        string retS =
            returnTypeToString(ret, p->returnIsOptional(), scope, p->getMetaData(), useWstring | TypeContextCpp11);
...
Slice::Gen::ForwardDeclVisitor::ForwardDeclVisitor(Output& h) : H(h), _useWstring(false) {}
...
Slice::Gen::ProxyVisitor::visitOperation(const OperationPtr& p)
{
    ...
    else
    {
        ...
        if (futureOutParams.size() > 1)
        {
            C << nl << "auto _responseCb = [_response = ::std::move(" << responseParam << ")](" << futureT
              << "&& _result)";
            C << sb;
    ...
    for (ParamDeclList::iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        ...
        else
        {
            ...
            string responseTypeS =
                inputTypeToString((*q)->type(), (*q)->optional(), interfaceScope, (*q)->getMetaData(), typeCtx);
