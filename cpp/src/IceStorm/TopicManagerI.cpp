...
namespace
{
    ...
    private:
        optional<TopicManagerPrx> getMaster(int64_t& generation, const char* file, int line) const
        {
            ...
            return uncheckedCast<TopicManagerPrx>(node ? node->startUpdate(generation, file, line) : nullopt);
        }
...
{
    TopicManagerSyncPrx sync = uncheckedCast<TopicManagerSyncPrx>(master);
