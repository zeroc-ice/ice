      ...
      _observers(make_shared<Observers>(_traceLevels))
{
    try
    {
        ...
        if (properties->getProperty(name + ".TopicManager.AdapterId").empty())
        {
            ...
            if (!p.empty())
            {
                const_cast<Ice::ObjectPrxPtr&>(_topicReplicaProxy) = _communicator->stringToProxy("dummy:" + p);
            }
            p = properties->getProperty(name + ".ReplicatedPublishEndpoints");
            if (!p.empty())
            {
                const_cast<Ice::ObjectPrxPtr&>(_publisherReplicaProxy) = _communicator->stringToProxy("dummy:" + p);
            }
        }
