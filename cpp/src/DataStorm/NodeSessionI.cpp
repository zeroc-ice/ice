...
namespace
{
    ...
    // This class implements the Slice DataContract::Node interface by forwarding calls to the target Node object using
    // the connection established during the creation of the NodeSession object. The NodeForwarder wraps the node and
    // session proxy parameters passed to the DataContract::Node operations in forwarder proxies, which handle
    // forwarding to the corresponding target objects.
