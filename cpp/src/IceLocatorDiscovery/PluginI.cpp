...
PluginI::initialize()
{
    ...
    _locator =
        make_shared<LocatorI>(_name, Ice::uncheckedCast<LookupPrx>(lookupPrx), properties, instanceName, voidLocator);
...
LocatorI::setLookupReply(const LookupReplyPrxPtr& lookupReply)
{
    ...
    for (vector<pair<LookupPrxPtr, LookupReplyPrxPtr>>::iterator p = _lookups.begin(); p != _lookups.end(); ++p)
    {
        Ice::UDPEndpointInfoPtr info =
            dynamic_pointer_cast<Ice::UDPEndpointInfo>(p->first->ice_getEndpoints()[0]->getInfo());
