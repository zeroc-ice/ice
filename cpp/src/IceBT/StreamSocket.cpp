...
IceBT::StreamSocket::setBufferSize(SOCKET fd, int rcvSize, int sndSize)
{
    ...
    if(rcvSize > 0)
    {
        ...
        if(size > 0 && size < rcvSize)
        {
            ...
            //
            IceInternal::BufWarnSizeInfo winfo = _instance->getBufWarnSize(BTEndpointType);
            if(!winfo.rcvWarn || rcvSize != winfo.rcvSize)
            {
                ...
                _instance->setRcvBufWarnSize(BTEndpointType, rcvSize);
            }
        }
    }
    ...
    if(sndSize > 0)
    {
        ...
        if(size > 0 && size < sndSize)
        {
            ...
            if(!winfo.sndWarn || sndSize != winfo.sndSize)
            {
                ...
                _instance->setSndBufWarnSize(BTEndpointType, sndSize);
            }
        }
    }
}
