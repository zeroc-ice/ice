...
namespace
{
    ...
    {
    public:
        GetFileInfoSeqAsyncCB(bool useSmallFileAPI) : _useSmallFileAPI(useSmallFileAPI)
    ...
    public:
        ...
        virtual bool checksum(const string& path) { return _feedback->checksumProgress(path); }
    ...
    void PatcherI::init(const FileServerPrxPtr& server)
    {
        if (_dataDir.empty())
        {
        ...
        const_cast<FileServerPrxPtr&>(_serverCompress) = Ice::uncheckedCast<FileServerPrx>(server->ice_compress(true));
        ...
        bool useSmallFileAPI)
    {
        if (useSmallFileAPI)
        {
            serverNoCompress->getFileCompressedAsync(
                path, static_cast<int32_t>(pos), chunkSize,
                    ...
                    try
                    {
                        ...
                        while (pos < p->size)
                        {
                            if (!curCB)
                            {
                                ...
                                getFileCompressed(_serverNoCompress, p->path, pos, _chunkSize, curCB, _useSmallFileAPI);
                            }
                            else
                            {
                                assert(nxtCB);
