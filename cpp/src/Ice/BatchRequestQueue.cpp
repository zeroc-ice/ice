...
namespace
{
    ...
    public:
        ...
        virtual void enqueue() const { _queue.enqueueBatchRequest(_proxy); }

        virtual int getSize() const { return _size; }

        virtual const std::string& getOperation() const { return _operation; }

        virtual const Ice::ObjectPrx& getProxy() const { return _proxy; }
      ...
      _batchRequestNum(0)
{
    ...
    if (_maxSize > 0 && datagram)
    {
        ...
        size_t udpSndSize = static_cast<size_t>(
...
void
BatchRequestQueue::finishBatchRequest(OutputStream* os, const Ice::ObjectPrx& proxy, const std::string& operation)
{
    //
