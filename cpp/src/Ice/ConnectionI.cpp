                ...
                while (true)
                {
                    ...
                    if (_readHeader)
                    {
                        ...
                        if (pos < headerSize)
                        {
                            ...
                            //
                            throw MarshalException{__FILE__, __LINE__, "received Ice message with too few bytes in header"};
                        }
                        ...
                        {
                            throw ProtocolException{__FILE__, __LINE__, "invalid protocol version in message header: " + Ice::protocolVersionToString(pv)};
                        }
                        EncodingVersion ev;
                        _readStream.read(ev);
                        if (ev != currentProtocolEncoding)
                        {
                            throw ProtocolException{__FILE__, __LINE__, "invalid protocol encoding version in message header: " + Ice::encodingVersionToString(ev)};
                        }
                        uint8_t messageType;
                        ...
                        if (size < headerSize)
                        {
                            throw MarshalException{__FILE__, __LINE__, "received Ice message with unexpected size " + to_string(size)};
                        }
...
Ice::ConnectionI::validate(SocketOperation operation)
{
    if (!_endpoint->datagram()) // Datagram connections are always implicitly validated.
    {
        ...
        else // The client side has the passive role for connection validation.
        {
            ...
            if (size != headerSize)
            {
                throw MarshalException{__FILE__, __LINE__, "received ValidateConnection message with unexpected size " + to_string(size)};
            }
...
Ice::ConnectionI::doCompress(OutputStream& uncompressed, OutputStream& compressed)
{
    ...
    if (bzError != BZ_OK)
    {
        throw ProtocolException{__FILE__, __LINE__, "cannot compress message - BZ2_bzBuffToBuffCompress failed" + getBZ2Error(bzError)};
    }
    compressed.b.resize(headerSize + sizeof(int32_t) + compressedLen);
...
Ice::ConnectionI::doUncompress(InputStream& compressed, InputStream& uncompressed)
{
    ...
    {
        throw MarshalException{__FILE__, __LINE__, "unexpected message size after uncompress: " + to_string(uncompressedSize)};
    }
    ...
    if (bzError != BZ_OK)
    {
        throw ProtocolException{__FILE__, __LINE__, "cannot decompress message - BZ2_bzBuffToBuffDecompress failed" + getBZ2Error(bzError)};
    }
        ...
        switch (messageType)
        {
            ...
            case requestBatchMsg:
            {
                ...
                else
                {
                    ...
                    {
                        requestCount = 0;
                        throw MarshalException{__FILE__, __LINE__, "received batch request with " + to_string(requestCount) + " batches"};
                    }
            ...
            default:
            {
                ...
                throw ProtocolException{__FILE__, __LINE__, "received Ice protocol message with unknown type: " + to_string(messageType)};
            }
        }
    }
    catch (const LocalException& ex)
