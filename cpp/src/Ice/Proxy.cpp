...
{
    RequestHandlerPtr handler = _invoker->getRequestHandler();
    if(handler)
    {
        try
        {
...
Ice::ObjectPrx::ObjectPrx(const ReferencePtr& reference) noexcept :
    _reference(reference),
    _invoker(make_shared<ProxyInvoker>(reference))
{
}
