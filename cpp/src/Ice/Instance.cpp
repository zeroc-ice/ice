...
IceInternal::Instance::setServerProcessProxy(const ObjectAdapterPtr& adminAdapter, const Identity& adminIdentity)
{
    ...
    if (locator && serverId != "")
    {
        ProcessPrx process = admin->ice_facet<ProcessPrx>("Process");
        try
        {
            //
...
IceInternal::Instance::finishSetup(int& argc, const char* argv[], const Ice::CommunicatorPtr& communicator)
{
    ...
    {
        optional<RouterPrx> router{communicator->propertyToProxy<RouterPrx>("Ice.Default.Router")};
        if (router)
        {
    ...
    {
        optional<LocatorPrx> locator{communicator->propertyToProxy<LocatorPrx>("Ice.Default.Locator")};
        if (locator)
        {
