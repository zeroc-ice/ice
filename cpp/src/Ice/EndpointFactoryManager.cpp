...
IceInternal::EndpointFactoryManager::create(string_view str, bool oaEndpoint) const
{
    ...
    // If the stringified endpoint is opaque, create an unknown endpoint, then see whether the type matches one of the
    // known endpoints.
    if (protocol == "opaque")
    {
        ...
        if (factory)
        {
            // Make a temporary stream, write the opaque endpoint data into the stream, and ask the factory to read the
            // endpoint data from that stream to create the actual endpoint.
            OutputStream bs;
            bs.write(ue->type());
            ue->streamWrite(&bs);
...
IceInternal::EndpointFactoryManager::read(InputStream* s) const
{
    ...
    // If the factory failed to read the endpoint, return an opaque endpoint. This can occur if for example the factory
    // delegates to another factory and this factory isn't available. In this case, the factory needs to make sure the
    // stream position is preserved for reading the opaque endpoint.
    if (!e)
    {
