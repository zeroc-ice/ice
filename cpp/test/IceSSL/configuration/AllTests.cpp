...
#    endif
    ...
    /* TODO: Fix this test
        cout << "testing protocols... " << flush;
        {
    #ifndef ICE_USE_SECURE_TRANSPORT
            {
                ...
                Test::ServerFactoryPrxPtr fact =
    Ice::checkedCast<Test::ServerFactoryPrx>(comm->stringToProxy(factoryRef)); test(fact); Test::Properties d =
    createServerProps(defaultProps, p12, "s_rsa_ca1", "cacert1"); d["IceSSL.VerifyPeer"] = "0"; d["IceSSL.Protocols"] =
    "tls1_2"; Test::ServerPrxPtr server = fact->createServer(d); try
                {
                    server->ice_ping();
                    test(false);
                }
    ...
    #   if !defined(OPENSSL_NO_SSL3_METHOD) && defined(SSL3_VERSION)
            ...
            //
            {
                ...
                Test::ServerFactoryPrxPtr fact =
    Ice::checkedCast<Test::ServerFactoryPrx>(comm->stringToProxy(factoryRef)); test(fact); Test::Properties d =
    createServerProps(defaultProps, p12, "s_rsa_ca1", "cacert1"); d["IceSSL.VerifyPeer"] = "0"; Test::ServerPrxPtr
    server = fact->createServer(d); try
                {
                    server->ice_ping();
                    test(false);
                }
    ...
    #   endif
            ...
            //     Test::ServerFactoryPrxPtr fact =
    Ice::checkedCast<Test::ServerFactoryPrx>(comm->stringToProxy(factoryRef));
            //     test(fact);
            //     Test::Properties d = createServerProps(defaultProps, p12, "s_rsa_ca1", "");
            //     d["IceSSL.VerifyPeer"] = "0";
...

            {
                ...
                Test::ServerFactoryPrxPtr fact =
    Ice::checkedCast<Test::ServerFactoryPrx>(comm->stringToProxy(factoryRef)); test(fact); Test::Properties d =
    createServerProps(defaultProps, p12); d["IceSSL.Ciphers"] = "(DH_anon*)"; d["IceSSL.VerifyPeer"] = "0";
    ...
    server = fact->createServer(d); try
            ...
            //
            {
                ...
                Test::ServerFactoryPrxPtr fact =
    Ice::checkedCast<Test::ServerFactoryPrx>(comm->stringToProxy(factoryRef)); test(fact); Test::Properties d =
    createServerProps(defaultProps, p12, "s_rsa_ca1", "cacert1"); d["IceSSL.VerifyPeer"] = "0";
                d["IceSSL.ProtocolVersionMin"] = "ssl3";
                Test::ServerPrxPtr server = fact->createServer(d);
                try
                {
...
#    endif
    ...
    cout << "testing system CAs... " << flush;
    {
...

        {
            ...
            CommunicatorPtr comm = initialize(initData);
            Ice::ObjectPrxPtr p =
                comm->stringToProxy("Glacier2/router:wss -p 443 -h zeroc.com -r /demo-proxy/chat/glacier2");
            while (true)
            {
                try
                {
