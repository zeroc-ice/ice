...
To build a C++ demo, you just need to open demo\demo.sln in
...
for example:
set ICE_HOME=C:\Ice-1.1.0\bin;%PATH%

Then open a Command Prompt, cd to the desired demo subdirectory,
...
set CLASSPATH=%ICE_HOME%\lib\Ice.jar;%ICE_HOME%\lib\db.jar
...
and enter 'java -cp classes:%CLASSPATH% Server' to run the server.
In a separate Command Prompt window, enter 'java -cp classes:%CLASSPATH%
