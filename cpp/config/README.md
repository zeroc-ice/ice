...
Use `DEPENDS` when a `.ice` source is itself generated by another build step. An `add_custom_command` in the same
directory is ordered automatically; anything else — such as a custom target in another directory — must be listed in
`DEPENDS` so it runs before the Slice compilation.
