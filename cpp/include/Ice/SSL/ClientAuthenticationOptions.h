...
namespace Ice::SSL
{
    ...
    struct SecureTransportClientAuthenticationOptions
    {
         ...
         *
         * This callback is invoked by the %SSL transport for each new outgoing connection before starting the %SSL
         * handshake to determine the appropriate client certificate chain. The callback must return a `CFArrayRef` that
         * represents the client's certificate chain, or nullptr if no certificate chain should be used for the
