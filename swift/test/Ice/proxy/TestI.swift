...
actor MyDerivedInterfaceI: MyDerivedInterface, Ice.Object {
    ...
    func ice_isA(id: String, current: Ice.Current) -> Bool {
        _ctx = current.ctx
        return Ice.DefaultObject<MyDerivedInterfaceTraits>().ice_isA(id: id, current: current)
    }

    func ice_id(current: Ice.Current) -> String {
        _ctx = current.ctx
        return Ice.DefaultObject<MyDerivedInterfaceTraits>().ice_id(current: current)
    }

    func ice_ids(current: Ice.Current) -> [String] {
        _ctx = current.ctx
        return Ice.DefaultObject<MyDerivedInterfaceTraits>().ice_ids(current: current)
    }
