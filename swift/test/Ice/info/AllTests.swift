...
func allTests(_ helper: TestHelper) async throws {
    ...
    do {
        ...
        try test(
            tcpEndpoint.type() == Ice.TCPEndpointType || tcpEndpoint.type() == Ice.SSLEndpointType
                || tcpEndpoint.type() == Ice.WSEndpointType || tcpEndpoint.type() == Ice.WSSEndpointType)
        ...
        communicator.getProperties().setProperty(
            key: "TestAdapter.Endpoints", value: "default -h * -p \(port)")
        communicator.getProperties().setProperty(
            key: "TestAdapter.PublishedEndpoints",
