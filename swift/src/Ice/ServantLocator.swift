...
/// You should consider using a default servant instead (see ``ObjectAdapter/addDefaultServant``).
/// For more advanced use cases, you can create a middleware (see ``ObjectAdapter/use``).
public protocol ServantLocator: Sendable {
    ...
    /// - Remark: If you call `locate` from your own code, you must also call ``finished``
