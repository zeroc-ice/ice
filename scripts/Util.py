...
class Result:
    getKey = (
        lambda self, current: (current.testcase, current.config)
        if isinstance(current, Driver.Current)
        else current
    )
    getDesc = (
        lambda self, current: current.desc
        if isinstance(current, Driver.Current)
        else ""
    )
...
class TestSuite(object):
    ...
    def isMainThreadOnly(self, driver):
        ...
        if isinstance(self.mapping, ObjCMapping) and not driver.getComponent().isCross(
            self.id
        ):
            return True
...
class ObjCMapping(CppBasedMapping):
    ...
    def getIOSAppFullPath(self, current):
        appName = (
            "Objective-C ARC Test Controller.app"
            if current.config.arc
            else "Objective-C Test Controller.app"
        )
        path = os.path.join(self.component.getTestDir(self), "ios", "controller")
        path = os.path.join(
            path,
